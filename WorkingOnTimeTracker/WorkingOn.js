    ...
    var user = window.loggedUser,
      ...
      urls = {
        ...
        assignables:
          ...
          "/api/v1/Users/" +
          user.id +
          "/Assignables?format=json&take=1000&include=[name,numericPriority,iteration[startDate,endDate],project,entityType,entityState[name,role,isInitial],assignments[generalUser,role]]&where=(entityState.isFinal eq 'false') and (entityType.name ne 'Feature') and (entityState.role is not null)",
        ...
        roleEffort: function (assignableId, roleId) {
          return (
            ...
            "/api/v1/Assignables/" +
            assignableId +
            "/RoleEfforts?format=json&include=[timeRemain]&where=(role.id eq " +
            roleId +
            ")"
          );
        },
    ...
    function createWorkingOnList() {
      function list(bus) {
        bus.on("contentRendered", function () {
          ...
          var $menuItem = $(
            '<div class="tau-main-menu__item"><span class="tau-popup-link i-role-workingon" title="Nothing">Working On</span></div>'
          ).on("click", function () {
            list.show(this);
          });
      ...
      list.stopWork = function (doneCB) {
        return function () {
          ...
          $.getJSON(
            urls["roleEffort"](workingOn.id, workingOn.role),
            function (data) {
              var remain = 0.0;
              ...
              var started = timeFmt.parse(workingOn.started),
                ended = new Date();
              (hours =
                Math.round((ended.getTime() - started.getTime()) / 36000.0) /
                100.0),
                (time = {
                  Description:
                    "Auto tracked from " +
              ...
              $.getJSON(
                urls["times"] +
                  "&where=(user.id eq " +
                  user.id +
                  ") and (assignable.id eq " +
                  workingOn.id +
                  ") and (role.id eq " +
                  workingOn.role +
                  ') and (date eq "' +
                  ended.toISOString().substr(0, 10) +
                  '")',
                function (data) {
                  ...
                  $.ajax({
                    ...
                    alert(
                      "Failed to post " +
                        ...
                        " hours to #" +
                        time.Assignable.Id +
                        ". Please input it on your timesheet manually."
                    );
                  });
                }
              );
                ...
                data: JSON.stringify({
                  userData: {
                    ...
                    recent: JSON.stringify(
                      workingOn.recent.slice(0, options.maxRecent)
                    ),
                  },
                  scope: "Public",
                }),
      ...
      list.loadActive = function (data) {
        ...
        if (typeof workingOn.id === "undefined" || workingOn.id === "") {
          container
            .find(".workingon-option.active")
            .addClass("empty")
            .off("click")
            .html('<span class="ghosted">Nothing</span>');
          $(".i-role-workingon").removeClass("active").attr("title", "Nothing");
          workingOn.id = null;
        } else {
          var option =
            '<em class="tau-entity-icon tau-entity-icon--' +
            workingOn.type +
            '">' +
            workingOn.id +
            "</em>" +
          ...
          $(".i-role-workingon")
            .addClass("active")
            .attr("title", "#" + workingOn.id + " " + workingOn.title);
        }
      ...
      list.loadAssignables = function (data) {
        //TODO: add lock to prevent multiple calls from stepping on one another
        var today = new Date(),
          tomorrow = new Date(
            today.getFullYear(),
            today.getMonth(),
        ...
        data.Items.forEach(function (a) {
          ...
          if (
            a.Iteration != null &&
            dateUtils.convertToTimezone(
              dateUtils.parse(a.Iteration.StartDate)
            )[0] >= tomorrow
          ) {
            return;
          }
          if (
            a.Iteration != null &&
            dateUtils.convertToTimezone(
              dateUtils.parse(a.Iteration.EndDate)
            )[0] < today
          ) {
            return;
          }
          ...
          a.Assignments.Items.forEach(function (assignedTo) {
            if (
              assignedTo.GeneralUser.Id == user.id &&
              assignedTo.Role.Id == a.EntityState.Role.Id
            ) {
              validItems[a.Id] = a;
            }
          });
          if (
            typeof validItems[a.Id] === "undefined" &&
            typeof a.Assignments.Next != "undefined"
          ) {
        ...
        var prioritize = function (a, b) {
          return a.NumericPriority > b.NumericPriority
            ? -1
            : a.NumericPriority < b.NumericPriority
            ? 1
            : 0;
        };
        ...
        items
          ...
          .attr("class", function (d) {
            return (
              "tau-entity-icon tau-entity-icon--" +
              d.EntityType.Name.toLowerCase()
            );
          })
          .text(function (d) {
            return String(d.Id);
