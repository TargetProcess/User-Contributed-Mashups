tau.mashups
 ...
 .addMashup(function ($, d3, topmenu, path, $deferred, dateUtils) {

    var workingOnList = function() {
        var user = window.loggedUser,
            ...
            urls = {'workingOn': path.get() + "/storage/v1/status/workingOn",
                    // Feature is the only assignable that cannot accept time entries (why?)
                    'assignables': path.get() + "/api/v1/Users/" + user.id + "/Assignables?format=json&take=1000&include=[name,numericPriority,iteration[startDate,endDate],project,entityType,entityState[name,role,isInitial],assignments[generalUser,role]]&where=(entityState.isFinal eq 'false') and (entityType.name ne 'Feature') and (entityState.role is not null)",
                    'times': path.get() + "/api/v1/Times?format=json",
                    'roleEffort': function(assignableId, roleId) { return path.get() + "/api/v1/Assignables/" + assignableId + "/RoleEfforts?format=json&include=[timeRemain]&where=(role.id eq " + roleId + ")"; }
                    },
            workingOn = {},
            options = {
                maxRecent: 5,
            ...
            bus.on('contentRendered', function() {
                if (!rendered) {
                    $('head').append('<style type="text/css">'
                                    ...
                                    +'.workingon-optoin.active.empty {cursor: default} '
                                    ...
                                    +'.workingon-option > * {margin-right: 5px} '
                                    ...
                                    +'.workingon-option .title {display: inline-block; white-space: nowrap; overflow: hidden; max-width: 286px} '
                                    +'.workingon-option.active .title {width: 135px} '
                                    +'.date-time {font-size: 10px; width: 138px; display: inline-block; margin: 0} '
                                    +'</style>');
                    $('.tau-menu-item-timesheet').append($('<span class="tau-popup-link i-role-workingon" title="Nothing">Working On</span>').on('click', function() {list.show(this);}));
                    $('div.tau-app-main-pane').append(container).append(overlay);
                    overlay.on('click', function() {list.hide();});
                    rendered = true;
                    $.getJSON(urls['workingOn'], list.loadActive);
                }
            });
            return list;
        }
        ...
        list.startWork = function(item) {
            ...
            function startCB() {
                ...
                $.ajax({ type: "POST",
                             ...
                             data: JSON.stringify({userData: {id: workingOn.id,
        ...
        list.stopWork = function(doneCB) {
            ...
            $.getJSON(urls['roleEffort'](workingOn.id, workingOn.role), function(data) {
                ...
                $.ajax({ type: "POST",
                            url: urls['workingOn'],
                            contentType: "application/json",
                            data: JSON.stringify({userData: {id: "", type: "", title: "", project: "", role: "", started: "", recent: JSON.stringify(workingOn.recent.slice(0, options.maxRecent))}})
                        }).done(function() {
                            $.getJSON(urls['workingOn'], list.loadActive);
                            if (doneCB != null) doneCB();
                        }).fail(function() {
        ...
        list.loadOptions = function(data) {
            if (typeof(data.maxRecent) != "undefined") options.maxRecent = data.maxRecent;
            if (typeof(data.projectFilter) != "undefined") options.projectFilter = JSON.parse(data.projectFilter);
        };
        ...
        list.loadActive = function(data) {
            ...
            if (typeof(workingOn.id) === "undefined" || workingOn.id === "") {
                container.find('.workingon-option.active').addClass('empty').off('click').html('<span class="ghosted">Nothing</span>');
                $('.i-role-workingon').removeClass('active').attr('title', 'Nothing');
                workingOn.id = null;
            } else {
                container.find('.workingon-option.active').removeClass('empty').off('click').html(
                    '<em class="ui-type-icon ui-type-icon-'+workingOn.type+'">'+workingOn.id+'</em><span class="title">'
                    +workingOn.title+'</span><span class="ghosted">Since:</span><span class="date-time">'+workingOn.started+'</span>')
                    .on('click', list.stopWork(null));
                $('.i-role-workingon').addClass('active').attr('title', '#'+workingOn.id+' '+workingOn.title);
            }
            if (typeof(workingOn.recent) === "undefined") {
                workingOn.recent = [];
            } else {
                workingOn.recent = JSON.parse(workingOn.recent);
            }

            if (container.css('display') != "none") $.getJSON(urls['assignables'], list.loadAssignables);
        };

        list.loadAssignables = function(data) { //TODO: add lock to prevent multiple calls from stepping on one another
            var today = new Date(),
                tomorrow = new Date(today.getFullYear(), today.getMonth(), today.getDate()+1);
            if (clearCached) validItems = {};
            data.Items.forEach(function(a) {
                if (String(a.Id) === String(workingOn.id)) return;
                if (a.Iteration != null && dateUtils.convertToTimezone(dateUtils.parse(a.Iteration.StartDate))[0] >= tomorrow) return;
                if (a.Iteration != null && dateUtils.convertToTimezone(dateUtils.parse(a.Iteration.EndDate))[0] < today) return;
                if (a.EntityState.IsInitial && a.Iteration == null) return;
                if (options.projectFilter.indexOf(a.Project.Id) >= 0) return;
            ...
            sections.append('h3').text(function(d) {return d.name;});
            var items = sections.append('div').attr('style', 'padding: 3px').selectAll('.workingon-option')
                            .data(function(d) {return d.items;}).enter()
                                .append('span')
                                    ...
                                    .attr('title', function(d) { return 'Project: ' + d.Project.Name + ', State: ' + d.EntityState.Name + ', Title: ' + d.Name; });
            items.append('em')
                .attr('class', function(d) { return 'ui-type-icon ui-type-icon-' + d.EntityType.Name.toLowerCase(); })
                .text(function(d) { return String(d.Id); });
            items.append('span')
                .attr('class', 'title')
        ...
        list.show = function(e) {
            var left = e.offsetLeft + e.offsetParent.offsetLeft + e.offsetParent.offsetParent.offsetLeft + Math.floor(e.clientWidth/2) - 288;
